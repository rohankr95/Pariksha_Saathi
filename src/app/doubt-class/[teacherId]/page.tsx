...
export default async function TeacherSlotsPage({ params }: { params: Promise<{ teacherId: string }> }) {
  ...
  const days = Array.from(grouped.entries()).map(([date, daySlots]) => ({
    ...
    slots: daySlots.map((s) => ({
      ...
      label: s.start.toLocaleTimeString("hi-IN", { hour: "numeric", minute: "2-digit", timeZone: "Asia/Kolkata" }),
      capacity: s.capacity,
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      ...
      <p className="mb-6 text-sm text-muted-foreground">{teacher.subjects.map((s) => s.nameHi).join(", ") || "सामान्य"}</p>
